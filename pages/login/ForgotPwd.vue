<template>
  <div class="register1">
    <el-header className="go_back" title="忘记密码">
    ...
    <section class="content">
      ...
      <div class="btn-s cl">
        ...
        <div class="login-btn" v-if="!agree ||!(phone.trim())||!(msgCode.trim())||!(password.trim())">修改密码</div>
        <div class="login-btn on" v-if="agree&&phone.trim()&&msgCode.trim()&&password.trim()" @click="resetPwd">修改密码</div>
      </div>
    </section>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      getMsgCode(){//获取短信验证码
        this.$http.post(this.API.resetSendSms, {
          phone: this.phone
        }).then(res => {
...
//      },
      resetPwd () { //点击注册
        Indicator.open();
        this.$http.post(this.API.resetPwd, {
          phone: this.phone,
